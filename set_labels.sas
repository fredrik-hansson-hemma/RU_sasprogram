...
* (sätter rubriker samt tabellbeskrivning på tabell Ekonomi.)
************************************/
...
%if "&VATABLE" ne "" and "&TABLEDESCRIPTION" eq "NULL" %then %do;
proc sql noprint;
SELECT VyBeskrivning into :tabledescription
 FROM &dwlib..Modelleringar_Vyer
WHERE upcase(vyNamn) = upcase("&vatable");
 quit;

 %if "&tabledescription" eq "NULL" %then %let tabledescription =;
...
* Exempel på anrop.;
%set_labels(vatable=AKUTLIGGAREN);
